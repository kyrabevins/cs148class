...
$admin = true;
$columns = 6;
    $query = 'SELECT fldTitle,fldAuthor,fldFirstName,fldLastName,fldRating,fldDescription FROM tblUsers,tblUsersBooks, tblBooks WHERE tblUsers.pmkEmail = tblUsersBooks.fnkEmail AND tblUsersBooks.fnkBookId=tblBooks.pmkBookId ORDER BY fldDateReviewed';
    ...
    foreach ($info2 as $rev) {
        ...
        print '<h3 class="title">Title: ' . '<a href="https://kbevins.w3.uvm.edu/cs148/assignment10/form.php?title=' . $rev["fldTitle"] . '">' .  $rev["fldTitle"] . '   [Edit]' . "</a><br>";
